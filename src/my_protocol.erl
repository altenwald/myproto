...
%      % here ConnectionId is a thread id
%     {ok, Hello, My} = my_protocol:hello(ConnectionId),
...
-record(my, {
    ...
    socket :: undefined | inet:socket(), %% When socket is set,
...
hello(ConnectionId, #my{} = My) when is_integer(ConnectionId) ->
    Hash = list_to_binary(
        lists:map(fun
            (0) -> 1;
            (X) -> X
        end, binary_to_list(crypto:strong_rand_bytes(20)))),
    Hello = #response{id = ConnectionId,
                      status = ?STATUS_HELLO,
                      info = Hash},
    send_or_reply(Hello, My#my{connection_id = ConnectionId,
                               hash = Hash,
                               state = auth,
                               id = 2}).
...
buffer_bytes(Bin, #my{buffer = Buffer} = My) when size(Buffer) > 0
                                             andalso size(Bin) > 0 ->
...
decode(#my{buffer = Bin, state = auth, hash = Hash} = My) when size(Bin) > 4 ->
    ...
    {ok, #request{info = #user{} = User} = Req, Rest} ->
        {ok, Req#request{info = User#user{server_hash = Hash}},
             My#my{state = normal, buffer = Rest}}
    end;

decode(#my{buffer = Bin,
           state = normal,
        ...
        Packet1 = case command_code(CommandCode) of
            'query' when ParseQuery ->
                ...
                SQL = case mysql_proto:parse(Query2) of
                    {fail, Expected} ->
                        {parse_error, {fail, Expected}, Info};
                    Parsed ->
                        Parsed
                end,
