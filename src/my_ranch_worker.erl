...
start_server(Port, Name, Handler, Args) ->
    ...
    ranch:start_listener(Name, 10, ranch_tcp, [
        {port, Port}, {backlog,4096}, {max_connections,32768}
    ], ?MODULE, [Handler, Args]).
...
start_link(ListenerPid, Socket, _Transport, [Handler, Args]) ->
    proc_lib:start_link(?MODULE, init_server,
        [ListenerPid, Socket, Handler, Args]).
...
handle_info({tcp_closed, _Socket},
        #server{handler=Handler, state=HandlerState}=Server) ->
    Handler:terminate(tcp_closed, HandlerState),
    {stop, normal, Server};

handle_info({tcp_error, _, Error},
        #server{handler = Handler, state = HandlerState} = Server) ->
    ...
    {ok, Query, My1} ->
        ...
        {reply, default, HandlerState1} ->
            {reply, Reply, HandlerState2} =
                default_reply(Query, Handler, HandlerState1),
...
default_reply(#request{info=#select{params=[#variable{name =
        <<"version_comment">>}]}}, Handler, State) ->
...
default_reply(#request{info=#select{params=[#variable{name =
        <<"global.max_allowed_packet">>}]}}, _Handler, State) ->
    Info = {
...
default_reply(#request{info = #select{params=[#variable{name =
        <<"tx_isolation">>, scope = local}]}}, _Handler, State) ->
    Info = {
...
default_reply(#request{info = #show{type = databases}}, Handler, State) ->
    ...
    ResponseFields = {[#column{
        name = <<"Database">>, type=?TYPE_VAR_STRING, length=20,
        schema = <<"information_schema">>, table = <<"SCHEMATA">>,
        org_table = <<"SCHEMATA">>, org_name = <<"SCHEMA_NAME">>
    }], [ [DB] || DB <- Databases]},
...
default_reply(#request{info = #show{type = variables}}, Handler, State) ->
    ...
    Timestamp = iolist_to_binary(
        io_lib:format("~B.~6..0B", [Mega*1000000 + Sec, Micro])),

    DefaultStorageEngine = application:get_env(myproto, default_storage_engine),
    Variables = [
    ...
    ResponseFields = {[
        #column{
            name = <<"Variable_name">>, type=?TYPE_VAR_STRING, length=20,
            schema = <<"information_schema">>, table = <<"SCHEMATA">>, 
            org_table = <<"SCHEMATA">>, org_name = <<"SCHEMA_NAME">>
        },
        #column{
            name = <<"Value">>, type=?TYPE_VAR_STRING, length=20,
            schema = <<"information_schema">>, table = <<"SCHEMATA">>,
            org_table = <<"SCHEMATA">>, org_name = <<"SCHEMA_NAME">>
        }
...
default_reply(#request{info = #select{params =
        [#function{name = <<"DATABASE">>}]}}, _Handler, State) ->
    ResponseFields = {[
        #column{
            name = <<"DATABASE()">>, type=?TYPE_VAR_STRING, length=102,
            flags = 0, decimals = 31
        }
    ], []},
    Response = #response{status=?STATUS_OK, info = ResponseFields},
...
default_reply(#request{info = #show{type = tables}}, Handler, State) ->
    ...
    ResponseFields = {[
        #column{
            name = <<"Tables_in_", DB/binary>>, type=?TYPE_VAR_STRING,
            schema = DB, table = <<"TABLE_NAMES">>, 
            org_table = <<"TABLE_NAMES">>, org_name = <<"TABLE_NAME">>, 
            flags = 256, length = 192, decimals = 0
        }
    ], [ [Table] || Table <- Tables] },
    Response = #response{status=?STATUS_OK, info = ResponseFields},
    {reply, Response, State1};
...
default_reply(#request{info = #show{type = fields, from = Table, full = Full}},
    ...
    Header = [
        #column{
            name = <<"Field">>, org_name = <<"COLUMN_NAME">>, 
            type = ?TYPE_VAR_STRING, table = <<"COLUMNS">>, 
            org_table = <<"COLUMNS">>, schema = <<"information_schema">>, 
            length = 192, flags = 256
        },
        #column{
            name = <<"Type">>, org_name = <<"COLUMN_TYPE">>,
            type = ?TYPE_BLOB, table = <<"COLUMNS">>, org_table = <<"COLUMNS">>,
            schema = <<"information_schema">>, length = 589815, flags = 256
        },
        #column{
            name = <<"Null">>, org_name = <<"IS_NULLABLE">>,
            type = ?TYPE_VAR_STRING, table = <<"COLUMNS">>,
            org_table = <<"COLUMNS">>, schema = <<"information_schema">>,
            length = 9, flags = 256
        },
        #column{
            name = <<"Key">>, org_name = <<"COLUMN_KEY">>,
            type = ?TYPE_VAR_STRING, table = <<"COLUMNS">>,
            org_table = <<"COLUMNS">>, schema = <<"information_schema">>,
            length = 9, flags = 256
        },
        #column{
            name = <<"Default">>, org_name = <<"COLUMN_DEFAULT">>,
            type = ?TYPE_BLOB, table = <<"COLUMNS">>, org_table = <<"COLUMNS">>,
            schema = <<"information_schema">>, length = 589815, flags = 256
        },
        #column{
            name = <<"Extra">>, org_name = <<"EXTRA">>, type = ?TYPE_VAR_STRING,
            table = <<"COLUMNS">>, org_table = <<"COLUMNS">>,
            schema = <<"information_schema">>, length = 90, flags = 256
        }
    ] ++ case Full of
        true -> [
            #column{
                name = <<"Collation">>, org_name = <<"COLLATION_NAME">>,
                type = ?TYPE_VAR_STRING, table = <<"COLUMNS">>,
                org_table = <<"COLUMNS">>, schema = <<"information_schema">>,
                length = 96, flags = 256
            },
            #column{
                name = <<"Privileges">>, org_name = <<"PRIVILEGES">>,
                type = ?TYPE_VAR_STRING, table = <<"COLUMNS">>,
                org_table = <<"COLUMNS">>, schema = <<"information_schema">>,
                length = 240, flags = 256
            },
            #column{
                name = <<"Comment">>, org_name = <<"COLUMN_COMMENT">>,
                type = ?TYPE_VAR_STRING, table = <<"COLUMNS">>,
                org_table = <<"COLUMNS">>, schema = <<"information_schema">>,
                length = 3072, flags = 256
            }
        ];
        false -> []
    end,
...
default_reply(#request{
        info = #show{type = create_table, from = Table}}, Handler, State) ->
    {reply, {_DB, Table, Fields}, State1} = Handler:metadata({fields,Table}, State),
    ...
    Response = {[
        #column{
            name = <<"Table">>, type = ?TYPE_VAR_STRING, flags = 256,
            decimals = 31, length = 192
        },
        #column{
            name = <<"Create Table">>, type = ?TYPE_VAR_STRING, flags = 256,
            decimals = 31, length = 3072
        }
    ], [ [Table, CreateTable] ]},
...
default_reply(#request{command = field_list, info = Table}, Handler, State) ->
    ...
    Reply = [#column{
        schema = DB, table = Table, org_table = Table, 
        name = to_b(Field), org_name = to_b(Field), length = 20,
        type = case Type of
